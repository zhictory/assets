...
function writeBLECharacteristicValue(data){
    window.hilink.writeBLECharacteristicValue(UUID_OR_Mac,notifyUuids[0].serviceUuid,notifyUuids[0].writeCharacteristicUuid, data,'writeBLECharacteristicValueCallBack');
...
function sendGroupData(page){
    ...
    text1.innerHTML = Math.floor((page + 1) / groupNum * 100);
    arrowTop.style.display="none";
    mainVersion.style.top= 0 + "rem";
...
function checkUpdate(){
    ...
    query.find().then((versionControls) => {
      ...
      if(remoteVersion1 > localVersion1){
            ...
            firmwarePoint2.style.left = 10.8 + "rem";
